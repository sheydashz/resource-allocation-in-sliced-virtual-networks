function [OBJ] = obj_func(X,z,H,say,input_size,offload,Alpha,Beta,F_s,P_max,t_max,lambda,services,Nservices,Nchannel,B,noise,Ncell,Nuser,cpu_req,f_l,service);
% save('params.mat')
load('augmented.mat','theta','delta','phi','zeta','eta','say')
[total_rate,update_z] = rate_restated(X,z,H,Ncell,Nuser,Nchannel);
OBJ=0;
%% Calculate delay
...
% for j=1:Ncell
%     for i=1:Nuser
%         index1=(j-1)*Nuser*Nchannel+(i-1)*Nchannel+1;
%         index2=(j-1)*Nuser+i;
%         obj2=obj2+theta(index2,1)+say*(sum(X(index1:index1+Nchannel))-P_max);
%     end
% end
% obj2= 1/(2*say)*(max(0,obj2))^2;
...
for j=1:Ncell
    for k=1:Nchannel 
        temp2=0;
        for i=1:Nuser
            index=index0+(j-1)*Nuser*Nchannel+(i-1)*Nchannel+k;
            temp2=temp2+X(index,1);
        end
        temp=temp+phi(k,j)+say*(temp2-1);
    end
end
obj4=1/(2*say)*(max(0,temp))^2-sum(sum(phi.^2));

%%% x_{u,n}-x_{u,n}^2
temp=0;
obj5=0;
for j=1:Ncell*Nuser
    index=index0+(j-1)*Nchannel;
    for k=1:Nchannel
        temp=temp+zeta(j,k)+say*(X(index+k,1)-X(index+k,1)^2);
    end
end
obj5=(max(0,temp))^2-sum(sum(zeta.^2));
...
for j=1:Ncell
    for i=1:Nuser
        for k=1:Nchannel
            ...
            temp=temp+eta(j,i,k)+say1*(X(index1,1)-X(index0+index1,1)*P_max);
        end
    end
end
obj6= 1/(2*say1)*max(0,temp)^2-sum(sum(sum(eta.^2)));
%%% 
